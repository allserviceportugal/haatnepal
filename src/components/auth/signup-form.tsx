  ...
  if (state.needsEmailConfirmation) {
    return (
      <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-4 text-sm text-emerald-800">
        Almost there — we sent a confirmation link to your email. Verify it, then log in.
        <div className="mt-3">
          <Link href={loginHref} className="font-semibold text-emerald-700 underline">
            Go to log in
          </Link>
        </div>
      </div>
    ...
    <form action={formAction} className="space-y-5">
      ...
      <div>
        ...
        <input
          ...
          className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-orange-300"
        />
        <p className="mt-1 text-xs text-slate-500">Shown to buyers/sellers as a trust signal.</p>
      </div>
        ...
        <div className="mt-2 grid grid-cols-2 gap-3">
          <label className="flex cursor-pointer flex-col rounded-xl border border-slate-200 px-4 py-3 text-sm has-[:checked]:border-orange-300 has-[:checked]:bg-orange-50">
            <span className="flex items-center gap-2 font-semibold text-slate-700">
              <input type="radio" name="accountType" value="individual" defaultChecked className="h-4 w-4" />
              Individual
            </span>
          ...
          <label className="flex cursor-pointer flex-col rounded-xl border border-slate-200 px-4 py-3 text-sm has-[:checked]:border-orange-300 has-[:checked]:bg-orange-50">
            <span className="flex items-center gap-2 font-semibold text-slate-700">
              <input type="radio" name="accountType" value="business" className="h-4 w-4" />
              Business
            </span>
